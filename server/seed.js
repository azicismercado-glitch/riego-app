...
async function seedDiagnosticoChamula(tecnicoId) {
  const data = {
    ...
    tiempoTotalMeses: '6',
    presupuesto: [
      { inversion: 'Red de riego por goteo en 19,1 ha', tipo: 'Riego presurizado (goteo)', monto: 'USD 130.000 + IVA', montoUSD: 130000 },
      { inversion: 'Defensa activa contra heladas por aspersión sub-arbórea', tipo: 'Sistema antiheladas', monto: 'USD 70.000 + IVA', montoUSD: 70000 },
      { inversion: 'Reservorio impermeabilizado (geomembrana) y cabezal de riego', tipo: 'Reservorio / represa', monto: 'USD 60.000 + IVA', montoUSD: 60000 },
      { inversion: 'Generación solar fotovoltaica (kit on-grid) para el bombeo', tipo: 'Paneles solares / energía', monto: 'USD 33.827,10 + IVA', montoUSD: 33827.10 }
    ],
  ...
  // SEED_VILLA_BONITA_SIGNED=true en el .env recrea la versión vieja (ya firmada
  // por las 3 instancias). Por defecto queda SIN FIRMAR, con todos los datos
  // completos, lista para hacer una demo en vivo del circuito de firmas.
  const yaFirmado = process.env.SEED_VILLA_BONITA_SIGNED === 'true';
  const informe = yaFirmado ? generateConformidadDraft(data) : '';

  const { rows } = await db.query(
    `INSERT INTO diagnosticos (data, doc_status, informe_conformidad, created_by)
     VALUES ($1,$2,$3,$4) RETURNING id`,
    [data, yaFirmado ? 'firmado_cfi' : 'borrador', informe, tecnicoId]
  );
  const id = rows[0].id;
  ...
  if (yaFirmado) {
    ...
    await db.query(
      `INSERT INTO historial (diagnostico_id, ts, usuario, evento, detalle, tipo) VALUES ($1,'2026-06-10 16:20:00-03','aperez','Diagnóstico creado','Relevamiento en finca Villa Bonita 2','ok')`,
      [id]
    );
    console.log('Diagnóstico de ejemplo "Villa Bonita 2" (SIN firmar, listo para demo) creado, id =', id);
  }
}
